...
@Component({
  selector: 'app-eiteam-management',
  templateUrl: './eiteam-management.component.html',
  styleUrls: ['./eiteam-management.component.css']
})
export class EiTeamManagementComponent implements OnInit {
  ...
  deleteMember(value: {user: User,  isAdmin: boolean, isAccepted: boolean, isAnswered: boolean, id: number }) {
    ...
    console.log(this.eiTeamMembers.filter(p => p.user.id == currentUser.id)[0].isAdmin);
    if (this.eiTeamMembers.filter(p => p.user.id == currentUser.id)[0].isAdmin) {
      ...
      this.notificationService.showErrorMessage("Only EI admin can manage team members!");
    }
  }
  ...
  changeItemEI(newItem: any) {
    this.selectedEIToTeamView = newItem;

    this.userService.getAllUserEisFullInfoByEIId(newItem.id).subscribe(res => {
      var newEiTeamMembers: {user: User,  isAdmin: boolean, isAccepted: boolean, isAnswered: boolean, id: number }[] = [];
      
      res.forEach(element => {
        newEiTeamMembers.push({ user: element.user ? element.user : {} as User,  isAdmin: element.isAdmin,
          isAccepted: element.isAccepted, isAnswered: element.isAnswered, id: element.id  });
      });

      this.eiTeamMembers = newEiTeamMembers;
    });
  }
